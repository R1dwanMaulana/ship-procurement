...
export const useExport = () => {
  ...
  const exportPDF = (poList: readonly PengajuanPO[], filterLabel = 'Semua PO') => {
    ...
    // Build rows tanpa nested template literal ternary
    ...
    poList.forEach((po) => {
      if (po.items.length === 0) {
        rowParts.push(
          ...
          + '<td>' + (po.namaKapal || '-') + '</td>'
          + '<td>' + po.status + '</td>'
          + '<td>' + po.tanggalPengajuan + '</td>'
          + '<td>' + (po.lokasiDocking || '-') + '</td>'
    ...
    const html = '<!DOCTYPE html>'
      ...
      + '<title>Laporan - Ship Procurement BSL & SPT</title>'
      + '<style>' + style + '</style></head><body>'
      + '<div class="header"><h1>Ship Procurement BSL & SPT</h1><p>Laporan Inventaris Pengadaan Barang</p></div>'
      + '<div class="meta">'
